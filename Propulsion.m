function [MTOW, thrust] = Propulsion (weight_wings, weight_fuselage, RegConst, thrust_to_weight, sensorWeight, sensorContainerWeight, n_sensor, weight_propulsion)
   ...
   % capacity
    for k = 0:100
        ...
        MTOW = weight_empty + n_sensor .* (sensorWeight + sensorContainerWeight); %Max Takeoff weight (N)
        % thrust = 70;
    ...
    MTOW = weight_empty + n_sensor .* (sensorWeight + sensorContainerWeight); %Max Takeoff weight (N) (pass weigh 3 oz 0.085 kg)
        % thrust = 70;
end